...
export class FluidSimulationDemoParticleSystem extends FluidSimulationDemoBase {
  ...
  protected async _run() {
    ...
    if (useGPU) {
      this._particleSystem = new BABYLON.GPUParticleSystem(
        "particles",
    ...
    this._particleSystem.render(); // make sure the pre-warm cycles are done in the GPU case (for CPU case it is done when calling start())

    this._particleSystem.renderAsFluid = renderAsFluid;
