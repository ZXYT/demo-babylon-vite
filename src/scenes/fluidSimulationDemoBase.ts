...
export class FluidSimulationDemoBase {
  ...
  public addCollisionSphere(
    ...
    radius: number,
    dragPlane: BABYLON.Nullable<BABYLON.Vector3> = new BABYLON.Vector3(0, 1, 0),
    ...
    const promise: Promise<[BABYLON.Nullable<BABYLON.Mesh>, ICollisionShape]> =
      dontCreateMesh
  ...
  protected _checkCollisions(particleRadius: number): void {
    ...
    for (let a = 0; a < this._fluidSim!.currentNumParticles; ++a) {
      ...
      for (let i = 0; i < this._collisionObjects.length; ++i) {
        ...
        const dist =
          shape.scale * shape.sdEvaluate(pos, ...shape.params) - particleRadius;
        if (dist < 0) {
          shape.computeNormal(pos, shape, normal);

          const restitution =
            shape.collisionRestitution ?? this._shapeCollisionRestitution;
          ...
          velocities[a * 3 + 0] =
            (velocities[a * 3 + 0] - 2 * dotvn * normal.x) * restitution;
          velocities[a * 3 + 1] =
            (velocities[a * 3 + 1] - 2 * dotvn * normal.y) * restitution;
          velocities[a * 3 + 2] =
            (velocities[a * 3 + 2] - 2 * dotvn * normal.z) * restitution;
